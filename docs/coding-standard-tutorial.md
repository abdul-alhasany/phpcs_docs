...
All sniffs in PHP*CodeSniffer must belong to a coding standard. A coding standard is a directory with a specific sub-directory structure and a ruleset.xml file, so we can create one very easily. Let's call our coding standard \_MyStandard*. Run the following commands to create the coding standard directory structure:
